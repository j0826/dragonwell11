 ...
 * @requires vm.hasJFR
 * @library /test/lib
 * @run main/othervm jdk.jfr.event.runtime.TestWispThreadParkEvent
 */

public class TestWispThreadParkEvent {
    private static final String EVENT_NAME = EventNames.WispThreadPark;
    ...
    static RecordedEvent testPark(Consumer<Blocker> parkOperation, Thread.State threadState) throws Exception {
        ...
        for (RecordedEvent event : events) {
            RecordedClass rc = event.getValue("parkBlockerClass");
            System.out.println(event);
            if (rc != null) {
                assertTrue(Blocker.class.toString().contains(rc.getName()));
            }
            foundEvent = event;
        }
        assertNotNull(foundEvent, "Correct event not found");
        return foundEvent;
    }
