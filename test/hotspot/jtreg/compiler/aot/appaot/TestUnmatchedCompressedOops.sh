...
# @test
# @summary app aot should check unmatched configuration
# @run shell TestUnmatchedCompressedOops.sh
...
public class TmpTestAppAOT{
    ...
    public static void run(URL url) throws Exception{
        ...
        if (loaded != 0) throw new Exception("API: failed to load aot library");
...
${JAOTC} --output ${AOT_LIB} -J-XX:-UseCompressedOops -J-Dgraal.PrintCompilation=true --verbose -J-cp -J${SUB_DIR} ${TEST_SUB_CLASS}.class > aotc.log
if [ $? != '0' ]
then
...
{
  # check aot lib is skipped by unmatched option
  echo "check aot lib"
  try_load_message=`grep "test.so" output.txt|grep "try to load"|wc -l`
  
  if [[ $try_load_message -ne 1 ]]; then
    echo "not try load aot lib"
    exit -1
  fi

  # check error message when loading aot library
  echo "check error message"
  error_message=`grep "error: UseCompressedOops" output.txt|grep test.so|wc -l`
  if [[ $error_message -ne 1 ]]; then
    echo "miss expected error message"
    exit -1
  fi

  # check skip message
  echo "check skip message"
  skip_message=`grep "skipped" output.txt|grep test.so|wc -l`
  ...
  # check api error
  echo "check error message from api"
  java_api_message=`grep "API: failed to load" output.txt|wc -l`
  if [[ $java_api_message -ne 1 ]]; then
    echo "miss expected error message"
    exit -1
  fi
