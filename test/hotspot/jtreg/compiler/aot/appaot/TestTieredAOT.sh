...
# @test
# @summary test tiered aot and it will trigger tier3 compilation after retirement
# @run shell TestTieredAOT.sh
...
${JAOTC} --output ${AOT_LIB} --compile-for-tiered -J-Dgraal.PrintCompilation=true --verbose -J-cp -J${SUB_DIR} ${TEST_SUB_CLASS}.class > aotc.log
if [ $? != '0' ]
then
...
function check_output()
{
  ...
  if [[ $aot_method_messages -eq 0 ]]; then
    echo "aot method T1.xadd is not executed"
    exit -1
  fi

  # check jit method messages
  echo "check jit compilation level"
  aot_method_messages=`grep "T1::xadd" output.txt|wc -l`
  jit_compile=0
  for comp_level in `grep "T1::xadd" output.txt|awk '{print $3}'`
  do
    jit_compile=1
    echo "comp level:" $comp_level
    if [[ $comp_level -ne 3 && $comp_level -ne 4 ]]; then
      echo "T1.xadd is not jit compile with level 3 or 4"
      exit -1
    fi
  done

  # check method is compiled with jit
  if [[ $jit_compile -eq 0 ]]; then
    echo "T1.xadd is not jit compiled"
    exit -1
  fi
  exit 0
}
